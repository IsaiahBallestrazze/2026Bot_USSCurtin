...
public class RobotContainer {
  ...
  SwerveInputStream driveDirectAngle = driveAngularVelocity.copy()
      .withControllerHeadingAxis(driverController::getRightX, // gives inputs from the right joystick for turning
          driverController::getRightY)
      .headingWhile(true);

  Command driveFieldOrientedDirectAngle = drivebase.driveFieldOriented(driveDirectAngle); // allows robot to move at
  ...
  private void configureBindings() { // default
    ...
    // driverController.b().toggleOnTrue(new RunCommand(() ->
    // System.out.println("toggleOnTrue")));
    ...
    ////////////////////
    // new JoystickButton(buttonBox, 5).onTrue(new RunCommand(() ->
    // turretSub.setFlywheelRPM(4600), turretSub)); //1600 near minimum
    // new JoystickButton(buttonBox, 5).onTrue(new RunCommand(() ->
    // intakeSub.AgitatorSet(-.5), intakeSub));
    // new JoystickButton(buttonBox, 5).onTrue(new RunCommand(() ->
    // shootersub.ShooterGroup(1,1), shootersub));
    // new JoystickButton(buttonBox, 5).onTrue(new RunCommand(() ->
    // turretSub.setFlywheelRPM(400), turretSub));



    // new JoystickButton(buttonBox, 1).toggleOnTrue(new RunCommand(() ->
    // climberSub.getClimberPosition(), climberSub));
    // new JoystickButton(buttonBox, 2).toggleOnTrue(new RunCommand(() ->
    // intakeSub.getIntakeArmPosition(), intakeSub));


    // new JoystickButton(buttonBox, 5).onTrue(new RunCommand(() ->
    // climberSub.ClimberMotorDown(), climberSub));
    // new JoystickButton(buttonBox, 5).onFalse(new RunCommand(() ->
    // climberSub.ClimberSetSpeed(-.5), climberSub));

    // new JoystickButton(buttonBox, 1).onTrue(new RunCommand(() ->
    // turretSub.setTurretspeed(.5), turretSub));
    // new JoystickButton(buttonBox, 1).onFalse(new RunCommand(() ->
    // turretSub.setTurretspeed(0), turretSub));

    // // new JoystickButton(buttonBox, 2).onTrue(new RunCommand(() ->
    // turretSub.setTurretspeed(1), turretSub));
    // // new JoystickButton(buttonBox, 2).onFalse(new RunCommand(() ->
    // turretSub.setTurretspeed(0), turretSub));

    // new JoystickButton(buttonBox, 3).onTrue(new RunCommand(() ->
    // turretSub.setTurretAngle((Math.toRadians(45)))));
    // new JoystickButton(buttonBox, 4).onTrue(new RunCommand(() ->
    // turretSub.setTurretAngle(Math.toRadians(135))));

    // //new JoystickButton(buttonBox, 10).onTrue(new RunCommand(() ->
    // turretSub.SetFieldPosition(0, 0, 0), turretSub));
    // //new JoystickButton(buttonBox, 10).onTrue(new RunCommand(() ->
    // System.out.println(drivebase.getGyroRaw()), turretSub));
    // new JoystickButton(buttonBox, 10).whileTrue(new RunCommand(() ->
    // drivebase.updateVisionOdometryReal(), turretSub));

    // new JoystickButton(buttonBox, 9).onTrue(new RunCommand(() ->
    // turretSub.printoutLimelightData(), turretSub));

    // new JoystickButton(buttonBox, 5).toggleOnTrue(new RunCommand(() ->
    // turretSub.getTurretPosition(), turretSub));
    // new JoystickButton(buttonBox, 6).onTrue(new RunCommand(() ->
    // turretSub.resetTurretEncoder(), turretSub));

    // //fear this single line of code
    // new JoystickButton(buttonBox, 8).toggleOnTrue(new RunCommand(() ->
    ...
    // turretSub.getTurretAngle(),
    // turretSub.getTurretPID(),
    // Math.toRadians(drivebase.getAnglesInverted()))))); //converts the gyro to
    // radians for math
